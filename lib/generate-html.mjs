...
<body>
  <div class="container">
    ...
    ${errorCount > 0 ? errorsSection : ''}
    
    ${report.fullInventory ? generateFullInventorySection(report.fullInventory) : ''}
  ...
  <script>
    // Highlight commit when accessed via anchor link
    function highlightCommitFromHash() {
      const hash = window.location.hash;
      if (hash && hash.length > 1) {
        const element = document.getElementById(hash.substring(1));
        if (element && element.classList.contains('commit')) {
          // Remove any existing highlights
          document.querySelectorAll('.commit.highlighted').forEach(commit => {
            commit.classList.remove('highlighted');
          });
          
          // Add highlight to the target commit
    ...
    document.addEventListener('DOMContentLoaded', highlightCommitFromHash);
    
    // Highlight when hash changes (e.g., clicking anchor links)
    window.addEventListener('hashchange', highlightCommitFromHash);
  </script>
</body>
</html>
  `;
        ...
        return `
          <div class="commit" id="${commit.hash}">
            <div>
              ...
              <a href="#${commit.hash}" class="commit-anchor" title="Link to this commit">🔗</a>
            </div>
...
/**
 * Generate HTML section for full inventory
...
const generateFullInventorySection = (inventory) => {
  ...
  const rows = inventory.map(dep => {
    const npmUrl = `https://www.npmjs.com/package/${dep.name}`;
    
    // Format versions with conflict warnings
    const oldVersionCell = dep.oldVersion ? 
      (dep.hasOldConflict ? 
        `<span style="color: var(--danger-color); font-weight: bold;" title="Multiple versions detected!">⚠️ ${dep.oldVersion}</span>` : 
        dep.oldVersion) : 
      '<span style="color: #95a5a6;">-</span>';
    
    const newVersionCell = dep.newVersion ? 
      (dep.hasNewConflict ? 
        `<span style="color: var(--danger-color); font-weight: bold;" title="Multiple versions detected!">⚠️ ${dep.newVersion}</span>` : 
        dep.newVersion) : 
      '<span style="color: #95a5a6;">-</span>';
    
    const oldLevelCell = dep.oldLevel || '<span style="color: #95a5a6;">-</span>';
    const newLevelCell = dep.newLevel || '<span style="color: #95a5a6;">-</span>';
    
    return `
      <tr>
        <td><a href="${npmUrl}" target="_blank">${dep.name}</a></td>
        <td>${oldVersionCell}</td>
        <td>${newVersionCell}</td>
        <td>${oldLevelCell}</td>
        <td>${newLevelCell}</td>
      </tr>
    `;
  ...
  return `
    <h2 id="full-inventory">📦 Full Dependency Inventory</h2>
    <p>
      Complete list of all ${inventory.length} dependencies found in both versions.
      ${oldConflicts > 0 || newConflicts > 0 ? 
        `<br><strong style="color: var(--danger-color);">⚠️ Version conflicts detected: ${oldConflicts} in old version, ${newConflicts} in new version</strong>` : 
        ''}
    </p>
    <details>
      <summary style="cursor: pointer; padding: 10px; background-color: var(--light-color); border-radius: 6px; margin: 10px 0; font-weight: bold;">
        Click to expand/collapse inventory table
      </summary>
