...
const generateChangelog = async (report, llmCommand) => {
  ...
  // Added dependencies (exclude devDependencies)
  ...
  // Upgraded dependencies (exclude devDependencies)
  ...
  // Removed dependencies (exclude devDependencies)
  ...
  // Modified dependencies (namespace changes, exclude devDependencies)
  ...
  if (report.changes.nested) {
    ...
    // Process nested changes (exclude devDependencies)
...
const generateChangeSection = async (change, llmCommand) => {
  ...
  if (change.type === 'added') {
    ...
    output += ` [${change.changeType} update]\n\n`;
  }
...
const generateNestedChangeSection = async (change, llmCommand) => {
  ...
  if (change.type === 'added') {
    ...
    const changeTypeText = (changeType) => changeType === 'unknown' ? '' : `[${changeType} update]`  
    
    output += `#### ⬆️ ${change.name} ${change.oldVersion} → ${change.newVersion} *(via ${change.parent})*`;
    output += ` ${changeTypeText(change.changeType)}\n\n`;
  }
...
const summarizeCommits = async (commits, llmCommand) => {
  ...
  const prompt = `Please summarize the following git commits into concise bullet points for a CHANGELOG. 
Focus on user-facing changes, bug fixes, and new features. Ignore internal refactoring unless it's significant.
Rules to follow:
 - only text in bullet points, no markdown
 - Use only non-numbered bullet points, and dont add text before or after the bullet points. 
 - If commits are similar enough, you can join them together.
 - Keep short bullet points.
 - Dont add commit hash or PR number
 - Dont use 'we', keep to third person

Here are the commits to format as markdown bullet points:
...
Provide a concise summary in bullet point format:`;
  ...
  return new Promise((resolve, reject) => {
    ...
    const child = spawn('ollama', ['run', 'qwen2.5-coder:3b', prompt], {
