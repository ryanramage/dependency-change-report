 ...
 * Matches GitHub's github-slugger algorithm: lowercase, strip characters that
 * are not letters, numbers, connector punctuation, spaces, or hyphens, then
 * replace spaces with hyphens.
 * @param {string} text - The heading text
 * @returns {string} - The slug suitable for use as a fragment identifier
 */
...
const generateUpgradedSection = (upgraded, changelogs, ciStatus = {}) => {
  ...
  upgraded.forEach(dep => {
    ...
    const changelogLink = changelogs[dep.name] ? 
      `[📋 View](#${githubSlug(`${dep.name}: ${dep.oldVersion} → ${dep.newVersion}`)})` : 
      'N/A';
...
const generateModifiedSection = (modified, changelogs = {}) => {
  ...
  modified.forEach(dep => {
    ...
    const changelogLink = changelogs[dep.newName] ? 
      `[📋 View](#${githubSlug(`${dep.oldName} → ${dep.newName}: ${dep.oldVersion} → ${dep.newVersion}`)})` : 
      'N/A';
  ...
  if (changelogDeps.length > 0) {
    ...
    changelogDeps.forEach(dep => {
      ...
      output += `#### ${dep.oldName} → ${dep.newName}: ${dep.oldVersion} → ${dep.newVersion}\n\n`;
      output += `**Repository:** [${repoUrl}](${repoUrl})\n`;
      output += `**Commits:** ${changelog.commits.length}\n\n`;
