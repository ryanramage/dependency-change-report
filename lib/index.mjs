 ...
 */
const getGitHubActionsStatus = async (repoUrl, version) => {
  try {
    ...
    // First, try to get the commit SHA for the version/tag
    let commitSha = null;
    ...
    console.log(`[CI] Found commit SHA: ${commitSha.substring(0, 7)}`);
...
const getChangelogs = async (upgradedDeps, newerVersionDir, reposDir) => {
  ...
  for (const dep of upgradedDeps) {
    ...
    if (repoUrl) {
      ...
      try {
        console.log(`Getting GitHub Actions status for ${dep.name} at version ${dep.newVersion} from ${cleanRepoUrl}`);
        const actionsStatus = await getGitHubActionsStatus(cleanRepoUrl, dep.newVersion);
        if (actionsStatus) {
