 ...
 */
const executeCommand = async (command, args, cwd, timeout = time_5min, operationDescription = null, enablePeriodicLogging = false) => {
 ...
 */
const cloneRepo = async (repoUrl, ref, targetDir, enablePeriodicLogging = false) => {
  ...
  try {
    ...
    await executeCommand('git', ['clone', '--quiet', '--depth=1', '--single-branch', '--branch', ref, repoUrl, targetDir], undefined, time_2min, `git clone of ${repoName} (${ref})`, enablePeriodicLogging);
  } catch (error) {
    ...
    try {
      ...
      await executeCommand('git', ['clone', '--quiet', repoUrl, targetDir], undefined, time_5min, `git clone of ${repoName} (full)`, enablePeriodicLogging);
 ...
 * @returns {Promise<void>}
 */
const installDependencies = async (dir, enablePeriodicLogging = false) => {
  try {
...
const getCommitHistory = async (repoUrl, oldVersion, newVersion, reposDir) => {
  ...
  try {
    ...
    try {
      // 2 minute timeout for very large repositories
      await executeCommand('git', ['clone', '--quiet', '--depth=1', '--single-branch', repoUrl, tempDir], undefined, time_2min, `git clone of ${packageName} for history`, false);
    } catch (error) {
