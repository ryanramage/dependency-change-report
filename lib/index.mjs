...
/**
 * Process a single dependency for changelog and CI status
 ...
 */
const processSingleDependency = async (dep, newerVersionDir, reposDir) => {
  const result = {
    name: dep.name,
    changelog: null,
    error: null,
...
const processInParallel = async (dependencies, newerVersionDir, reposDir, concurrency = 5) => {
  ...
  for (let i = 0; i < dependencies.length; i += concurrency) {
    ...
    console.log(`Processing batch ${Math.floor(i / concurrency) + 1}/${Math.ceil(dependencies.length / concurrency)} (${batch.length} dependencies)`);
    
    const batchPromises = batch.map(dep => 
      processSingleDependency(dep, newerVersionDir, reposDir)
    );
