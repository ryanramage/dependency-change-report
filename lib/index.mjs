 ...
 */
const processSingleDependency = async (dep, newerVersionDir, reposDir, multibar) => {
  const result = {
    name: dep.name,
    changelog: null,
    error: null,
  ...
  const depBar = multibar.create(100, 0, { name: dep.name.padEnd(25), status: 'Starting...' });
...
const processInParallel = async (dependencies, newerVersionDir, reposDir, concurrency = 5) => {
  ...
  for (let i = 0; i < dependencies.length; i += concurrency) {
    ...
    const batchPromises = batch.map(dep => 
      processSingleDependency(dep, newerVersionDir, reposDir, multibar)
    );
