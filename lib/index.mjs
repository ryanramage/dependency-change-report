 ...
 */
const executeCommand = async (command, args, cwd, timeout = 300000) => {
  try {
...
const cloneRepo = async (repoUrl, ref, targetDir) => {
  try {
    ...
    // 10 minute timeout for very large repositories
    await executeCommand('git', ['clone', '--quiet', '--depth=1', '--single-branch', '--branch', ref, repoUrl, targetDir], undefined, 600000);
  } catch (error) {
    // If shallow clone with specific branch fails, try traditional approach
    console.warn(`Shallow clone failed, trying full clone: ${error.message}`);
    try {
      // Full clone with 15 minute timeout for very large repos
      await executeCommand('git', ['clone', '--quiet', repoUrl, targetDir], undefined, 900000);
      await executeCommand('git', ['checkout', ref], targetDir, 60000);
    } catch (fallbackError) {
      console.warn(`Warning: Failed to clone or checkout repository: ${fallbackError.message}`);
      throw fallbackError;
    }
  }
};
...
const getCommitHistory = async (repoUrl, oldVersion, newVersion, reposDir) => {
  try {
    ...
    try {
      // 10 minute timeout for very large repositories
      await executeCommand('git', ['clone', '--quiet', '--depth=1', '--single-branch', repoUrl, tempDir], undefined, 600000);
    } catch (error) {
    ...
    try {
      // 5 minute timeout for fetching tags from large repositories
      await executeCommand('git', ['fetch', '--tags', '--force', '--unshallow'], tempDir, 300000);
    } catch (error) {
 ...
 */
const processSingleDependency = async (dep, newerVersionDir, reposDir, updateProgress) => {
  const result = {
    name: dep.name,
    changelog: null,
    error: null,
  ...
  updateProgress(dep.name, 'starting', 'Getting repository URL...');
  
  const packageDir = join(newerVersionDir, 'node_modules', dep.name);
  const repoUrl = await getRepositoryUrl(packageDir);
  
  if (!repoUrl) {
    updateProgress(dep.name, 'error', 'No repository URL found');
    result.error = {
      repoUrl: null,
  ...
  updateProgress(dep.name, 'processing', 'Cleaning repository URL...');
  ...
  updateProgress(dep.name, 'processing', `Cloning repository and getting commits (${dep.oldVersion} → ${dep.newVersion})...`);
  
  let commits = [];
  try {
    commits = await getCommitHistory(cleanRepoUrl, dep.oldVersion, dep.newVersion, reposDir);
    if (commits.length > 0) {
      updateProgress(dep.name, 'processing', `Found ${commits.length} commits, getting CI status...`);
      result.changelog = {
      ...
      updateProgress(dep.name, 'warning', 'No commits found between versions');
      result.error = {
        repoUrl: cleanRepoUrl,
        oldVersion: dep.oldVersion,
        newVersion: dep.newVersion,
    ...
    updateProgress(dep.name, 'error', `Changelog error: ${error.message}`);
    result.error = {
      repoUrl: cleanRepoUrl,
      oldVersion: dep.oldVersion,
      newVersion: dep.newVersion,
  ...
  try {
    ...
    updateProgress(dep.name, 'processing', 'Getting GitHub Actions status...');
    const actionsStatus = await getGitHubActionsStatus(cleanRepoUrl, dep.newVersion, best);
    if (actionsStatus) {
      updateProgress(dep.name, 'success', `✓ Complete (CI: ${actionsStatus.status})`);
      result.ciStatus = actionsStatus;
    } else {
      updateProgress(dep.name, 'success', '✓ Complete (no CI data)');
    }
  } catch (error) {
    updateProgress(dep.name, 'success', '✓ Complete (CI error)');
  }
...
/**
 * Create a progress tracker for dependency processing
 * @param {number} totalDeps - Total number of dependencies
 * @returns {Object} - Progress tracker with update function
 */
const createProgressTracker = (totalDeps) => {
  const progressLines = new Map();
  let completedCount = 0;
  
  const updateProgress = (depName, status, message) => {
    const statusEmoji = {
      starting: '🔄',
      processing: '⚙️',
      warning: '⚠️',
    ...
    const truncatedName = depName.length > 25 ? depName.substring(0, 22) + '...' : depName;
    const truncatedMessage = message.length > 50 ? message.substring(0, 47) + '...' : message;
    
    // If this is a completion status, increment counter
    if ((status === 'success' || status === 'error') && !progressLines.has(depName)) {
      completedCount++;
    }
    
    const progressBar = `[${completedCount}/${totalDeps}]`;
    const line = `${progressBar} ${emoji} ${truncatedName.padEnd(25)} ${truncatedMessage}`;
    
    progressLines.set(depName, line);
    
    // Clear previous lines and redraw all active progress lines
    if (progressLines.size > 0) {
      // Move cursor up to overwrite previous lines
      const linesToClear = Math.min(progressLines.size, 10); // Limit to 10 visible lines
      ...
      // Show the most recent lines (up to 10)
      const recentLines = Array.from(progressLines.values()).slice(-10);
  ...
  const finish = () => {
    console.log(`\n✅ Completed processing ${totalDeps} dependencies\n`);
  };
...
const processInParallel = async (dependencies, newerVersionDir, reposDir, concurrency = 5) => {
  ...
  const { updateProgress, finish } = createProgressTracker(dependencies.length);
  
  console.log(`\nProcessing ${dependencies.length} dependencies with concurrency limit of ${concurrency}:\n`);
  
  // Process dependencies in batches with concurrency limit
  for (let i = 0; i < dependencies.length; i += concurrency) {
    ...
    const batchPromises = batch.map(dep => 
      processSingleDependency(dep, newerVersionDir, reposDir, updateProgress)
    );
