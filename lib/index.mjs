 ...
 * @param {string} reposDir - Repository directory
 * @param {Object} progressBar - CLI progress bar instance
 * @returns {Promise<Object>} - Object with changelog, error, and CI status
 */
const processSingleDependency = async (dep, newerVersionDir, reposDir, progressBar) => {
  const result = {
    name: dep.name,
    changelog: null,
    error: null,
...
const processInParallel = async (dependencies, newerVersionDir, reposDir, concurrency = 5) => {
  ...
  const progressBar = new cliProgress.SingleBar({
    format: 'Processing Dependencies |{bar}| {percentage}% | {value}/{total} | {name}',
    barCompleteChar: '\u2588',
    barIncompleteChar: '\u2591',
    hideCursor: true
  });
  
  progressBar.start(dependencies.length, 0, { name: 'Starting...' });
  ...
  for (let i = 0; i < dependencies.length; i += concurrency) {
    ...
    const batchPromises = batch.map(async (dep) => {
      const result = await processSingleDependency(dep, newerVersionDir, reposDir, progressBar);
      completed++;
      progressBar.update(completed, { name: dep.name });
      return result;
    });
  ...
  progressBar.stop();
  console.log(`✅ Completed processing ${dependencies.length} dependencies\n`);
