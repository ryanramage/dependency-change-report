  ...
  try {
    ...
    if (!currentRef) {
      ...
      try {
        ...
        if (currentRef === 'HEAD') {
          try {
            const { stdout: tagOutput } = await executeCommand('git', ['describe', '--tags', '--exact-match', 'HEAD'], repoPath, 30000, 'getting current tag');
            currentRef = tagOutput.trim();
          } catch {
            // If no exact tag match, use commit SHA
            const { stdout: shaOutput } = await executeCommand('git', ['rev-parse', 'HEAD'], repoPath, 30000, 'getting current commit');
    ...
    const { stdout: allTagsOutput } = await executeCommand('git', ['tag', '-l', '--sort=-version:refname'], repoPath, 30000, 'getting git tags');
