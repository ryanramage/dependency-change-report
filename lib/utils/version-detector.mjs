...
/**
 * Detects the current ("newer") git reference being built.
 * Prefers GitHub Actions environment, falling back to local git state.
 * Resolves PR merge refs to the actual HEAD commit.
 * @param {string} repoPath - Path to the git repository
 * @returns {Promise<string>} The current reference (tag, branch, or short SHA)
 */
export const detectCurrentRef = async (repoPath = '.') => {
...
export const detectVersions = async (repoPath = '.') => {
  try {
    const currentRef = await detectCurrentRef(repoPath);
