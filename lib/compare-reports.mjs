...
/**
 * Build request headers for fetching a remote report. Adds an Authorization
 * header from DCR_TOKEN (or GITHUB_TOKEN) so private reports can be fetched by
 * URL. For GitHub Contents API URLs it also requests the raw representation.
 * Without a token this is a no-op, preserving the original public-URL behavior.
 * @param {string} url
 * @returns {Record<string, string>}
 */
const buildFetchHeaders = (url) => {
  const headers = {};
  const token = process.env.DCR_TOKEN || process.env.GITHUB_TOKEN;
  if (token) {
  ...
  try {
    ...
    // Not a parseable URL host; skip GitHub-specific headers
  }
  return headers;
};
...
const loadReport = async (jsonPath) => {
  try {
    let content;
    if (isUrl(jsonPath)) {
      const response = await fetch(jsonPath, { headers: buildFetchHeaders(jsonPath) });
      if (!response.ok) {
