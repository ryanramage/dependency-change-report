...
export const buildDependencyGraph = (lockData) => {
  ...
  console.log(`DEBUG: Building dependency graph, lockfileVersion: ${lockData.lockfileVersion}`);

  if (lockData.lockfileVersion >= 2 && lockData.packages) {
    // Version 2+ format
    console.log(`DEBUG: Processing ${Object.keys(lockData.packages).length} packages`);
    
    for (const [path, info] of Object.entries(lockData.packages)) {
      if (path === '') {
        // Root package - track its direct dependencies as top-level
        if (info.dependencies) {
          console.log(`DEBUG: Root dependencies:`, Object.keys(info.dependencies));
          for (const depName of Object.keys(info.dependencies)) {
            graph.topLevel.add(depName);
          }
        }
        if (info.devDependencies) {
          console.log(`DEBUG: Root devDependencies:`, Object.keys(info.devDependencies));
      ...
      if (pathParts.length === 2) {
        ...
        console.log(`DEBUG: ${packageName} is top-level (path: ${path})`);
        ...
        console.log(`DEBUG: ${packageName} has parent ${parentName} (path: ${path})`);
    ...
    console.log(`DEBUG: Using lockfile v1 format`);
  ...
  // For lockfile v2+, also try to build relationships from dependency info in packages
  if (lockData.lockfileVersion >= 2 && lockData.packages) {
    console.log(`DEBUG: Building additional relationships from package dependencies`);
    for (const [path, info] of Object.entries(lockData.packages)) {
      if (path === '' || !info.dependencies) continue;
      
      const packageName = path.replace(/^node_modules\//, '');
      
      // Add relationships based on the dependencies field
      for (const depName of Object.keys(info.dependencies)) {
        // Initialize maps if needed
        if (!graph.parents.has(depName)) {
          graph.parents.set(depName, new Set());
        }
        if (!graph.children.has(packageName)) {
          graph.children.set(packageName, new Set());
        }
        ...
        // Add parent-child relationship
        graph.parents.get(depName).add(packageName);
        graph.children.get(packageName).add(depName);
        
        console.log(`DEBUG: Added relationship: ${depName} is child of ${packageName}`);
      }
    }
  }

  console.log(`DEBUG: Graph built - topLevel: ${graph.topLevel.size}, total packages with parents: ${graph.parents.size}`);
  console.log(`DEBUG: Top-level packages:`, Array.from(graph.topLevel));
