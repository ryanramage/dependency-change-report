...
import { pruneNodeModulesToPackageJson } from '../utils/prune-node-modules.mjs';
...
import { getRepositoryUrl } from '../npm/package-info.mjs';
import { normalizeVersion } from '../utils/repo-url.mjs';
 ...
 */
const processSingleDependency = async (dep, newerVersionDir, reposDir, multibar, maxNameLength, useProgressBars = true, quiet = false, cloneCache = null) => {
  ...
  // Plain per-dependency logging: only when we have neither a progress bar nor
  // quiet mode. Quiet mode (parallel analyses) processes silently; errors are
  // still captured in the returned result and summarized by the caller.
  const logPlain = !useProgressBars && !quiet;
  ...
  const { url: cleanRepoUrl, directory } = await getRepositoryUrl(packageDir);
  ...
  // Normalize versions (strip ^/~ ranges). A missing baseline/target version
  // (e.g. an added dependency) has no meaningful range — skip fast rather than
  // attempting a clone + range that can't succeed.
  const oldV = normalizeVersion(dep.oldVersion);
  const newV = normalizeVersion(dep.newVersion);
  if (!oldV || !newV) {
    if (depBar) {
      depBar.update(100, { status: '⏭️ no version range' });
    } else if (logPlain) {
      console.log(`  ⏭️ ${dep.name}: skipped (missing ${!oldV ? 'baseline' : 'target'} version)`);
    }
    result.error = {
      repoUrl: cleanRepoUrl,
      oldVersion: dep.oldVersion,
      newVersion: dep.newVersion,
      error: `Skipped: missing ${!oldV ? 'baseline' : 'target'} version`,
      category: 'no_version'
    };
    return result;
  }
  ...
  try {
    ...
    commits = await getCommitHistory(cleanRepoUrl, oldV, newV, reposDir, { directory, cloneCache });
    if (commits.length > 0) {
      if (depBar) {
      ...
      result.error = {
        ...
        oldVersion: oldV,
        newVersion: newV,
        error: `No commits found between versions ${oldV} -> ${newV}`
      };
    }
  } catch (error) {
  ...
  try {
    ...
    const actionsStatus = await getGitHubActionsStatus(cleanRepoUrl, newV, best);
    if (actionsStatus) {
 ...
 */
const processInParallel = async (dependencies, newerVersionDir, reposDir, concurrency = 5, quiet = false, cloneCache = null) => {
  ...
  // Check if we should use progress bars. When quiet (parallel analyses),
  // skip the interactive multibar entirely and fall back to plain logging so
  // concurrent analyses don't fight over the terminal.
  const useProgressBars = quiet ? false : shouldUseProgressBars(dependencies.length);
  ...
  const multibar = quiet ? null : createMultiProgressBar(dependencies.length);
  ...
  const promises = dependencies.map(dep => 
    queue.add(() => processSingleDependency(dep, newerVersionDir, reposDir, multibar, maxNameLength, useProgressBars, quiet, cloneCache))
  );
 ...
 */
const getChangelogs = async (upgradedDeps, newerVersionDir, reposDir, { concurrency = 5, quiet = false, cloneCache = null } = {}) => {
  return await processInParallel(upgradedDeps, newerVersionDir, reposDir, concurrency, quiet, cloneCache);
};
 ...
 * @returns {Promise<void>}
 */
const cleanupStaleWorktrees = async (repoDir = process.cwd()) => {
  console.log('🧹 Cleaning up stale worktrees...');
  try {
    // First, prune worktrees that no longer exist
    await executeCommand('git', ['worktree', 'prune'], repoDir, time_1min, 'pruning stale worktrees', false);
    
    // List all worktrees to see what's left
    const worktreeList = await executeCommand('git', ['worktree', 'list', '--porcelain'], repoDir, time_1min, 'listing worktrees', false);
    ...
    for (const worktree of worktreesToRemove) {
      try {
        ...
        await executeCommand('git', ['worktree', 'remove', '--force', worktree.path], repoDir, time_1min, `removing worktree ${worktree.branch}`, false);
      } catch (error) {
    ...
    await executeCommand('git', ['worktree', 'prune'], repoDir, time_1min, 'final prune', false);
 ...
 */
export const analyzeDependencyChanges = async (repoUrl, olderVersion, newerVersion, workingDir = process.cwd(), namespace = null, ignoreDevDependencies = false, debugTree = false, cleanupWorktrees = false, generateFullInventory = true, options = {}) => {
  // Setup signal handlers for graceful shutdown
  setupSignalHandlers();

  // The git repository to analyze (defaults to cwd for backward compatibility)
  // and any extra ignore rules (e.g. from .dcr.json) to merge with .dcrignore.
  const repoDir = options.repoDir || process.cwd();
  const extraIgnore = options.extraIgnore || { exactMatches: new Set(), patterns: [] };
  // Suppress the interactive multibar (use plain logging) and cap changelog
  // scraping concurrency — used when running multiple analyses in parallel so
  // they don't fight over the terminal or overwhelm the GitHub rate limit.
  const quietProgress = options.quietProgress || false;
  const changelogConcurrency = options.changelogConcurrency || 5;
  // Dedup dependency-repo clones across all changelog batches (and concurrent
  // deps): each repo is cloned once and shared — monorepo siblings that resolve
  // to the same repo no longer race on one clone dir or re-clone it N times.
  const changelogCloneCache = new Map();
  // Minimize on-disk node_modules: after each version's metadata is captured,
  // drop older's tree entirely and prune newer's to just package.json files.
  const minimizeDisk = options.minimizeDisk || false;

  // Clean up stale worktrees if requested
  if (cleanupWorktrees) {
    await cleanupStaleWorktrees(repoDir);
  }
  ...
  try {
    ...
    // Ensure the version directories' parents exist. Refs can contain slashes
    // (e.g. a branch like "release/v4.18"), which nest the version dir; without
    // this the subsequent `cp`/`git worktree add` fails on the missing parent.
    await mkdir(dirname(olderVersionDir), { recursive: true });
    await mkdir(dirname(newerVersionDir), { recursive: true });
    ...
    try {
      await executeCommand('git', ['fetch', 'origin', olderVersion], repoDir, time_2min, `git fetch ${olderVersion}`, isSmallScreen);
    } catch (error) {
      // Try fetching as a tag if direct fetch fails
      try {
        await executeCommand('git', ['fetch', 'origin', `refs/tags/${olderVersion}:refs/tags/${olderVersion}`], repoDir, time_2min, `git fetch tag ${olderVersion}`, isSmallScreen);
      } catch (tagError) {
    ...
    try {
      await executeCommand('git', ['fetch', 'origin', newerVersion], repoDir, time_2min, `git fetch ${newerVersion}`, isSmallScreen);
    } catch (error) {
      // Try fetching as a tag if direct fetch fails
      try {
        await executeCommand('git', ['fetch', 'origin', `refs/tags/${newerVersion}:refs/tags/${newerVersion}`], repoDir, time_2min, `git fetch tag ${newerVersion}`, isSmallScreen);
      } catch (tagError) {
        console.warn(`Warning: Could not fetch ${newerVersion}, will try to use existing ref`);
      }
    }
    
    // Resolve each version to an explicit commit, preferring refs the fetches
    // above just updated (tags, then remote-tracking branches). Without this,
    // a stale local branch with the same name — created by a previous run's
    // `git worktree add`/`git checkout` in a reused clone — shadows the fresh
    // remote ref and the report silently analyzes an old commit.
    const resolveWorktreeRef = async (version) => {
      for (const candidate of [`refs/tags/${version}`, `refs/remotes/origin/${version}`]) {
        try {
          const sha = (await executeCommand('git', ['rev-parse', '--verify', '--quiet', `${candidate}^{commit}`], repoDir, time_1min, `resolve ${candidate}`, false)).trim();
          if (sha) {
            console.log(`Resolved ${version} to ${sha.substring(0, 9)} (${candidate})`);
            return sha;
          }
        } catch (error) {
          // Candidate ref doesn't exist — try the next one.
        }
      }
      // Not a tag or remote branch: a raw SHA or local-only ref. Use as-is.
      return version;
    };
    const olderRef = await resolveWorktreeRef(olderVersion);
    const newerRef = await resolveWorktreeRef(newerVersion);

    // Create worktrees for each version, handling the case where one version is already checked out
    const currentBranch = await executeCommand('git', ['rev-parse', '--abbrev-ref', 'HEAD'], repoDir, time_1min, 'get current branch', false);
    ...
    // The copy-working-dir paths below intentionally use the checkout as-is
    // (it may hold uncommitted changes). Warn when it doesn't match the
    // freshly fetched ref, so a stale checkout is at least visible.
    if (olderIsCurrentBranch || newerIsCurrentBranch) {
      const copiedRef = olderIsCurrentBranch ? olderRef : newerRef;
      const copiedVersion = olderIsCurrentBranch ? olderVersion : newerVersion;
      try {
        const headSha = (await executeCommand('git', ['rev-parse', 'HEAD'], repoDir, time_1min, 'get HEAD sha', false)).trim();
        const targetSha = (await executeCommand('git', ['rev-parse', `${copiedRef}^{commit}`], repoDir, time_1min, `resolve ${copiedVersion}`, false)).trim();
        if (headSha !== targetSha) {
          console.warn(`⚠️  Working copy for ${copiedVersion} is at ${headSha.substring(0, 9)} but the fetched ref points to ${targetSha.substring(0, 9)} — the checkout may be stale (analyzing the working copy as-is)`);
        }
      } catch (error) {
    ...
    if (olderIsCurrentBranch) {
      ...
      await executeCommand('cp', ['-r', '.', olderVersionDir], repoDir, time_1min, `copy current dir to ${olderVersion}`, isSmallScreen);
      // Remove the .git directory from the copy to avoid conflicts
      await executeCommand('rm', ['-rf', `${olderVersionDir}/.git`], repoDir, time_1min, 'cleanup git dir', false);
      
      // Create worktree for newer version only
      console.log(`Creating worktree for ${newerVersion}...`);
      await executeCommand('git', ['worktree', 'add', '--detach', newerVersionDir, newerRef], repoDir, time_1min, `git worktree add ${newerVersion}`, isSmallScreen);
      ...
      await executeCommand('cp', ['-r', '.', newerVersionDir], repoDir, time_1min, `copy current dir to ${newerVersion}`, isSmallScreen);
      // Remove the .git directory from the copy to avoid conflicts
      await executeCommand('rm', ['-rf', `${newerVersionDir}/.git`], repoDir, time_1min, 'cleanup git dir', false);
      
      // Create worktree for older version only
      console.log(`Creating worktree for ${olderVersion}...`);
      await executeCommand('git', ['worktree', 'add', '--detach', olderVersionDir, olderRef], repoDir, time_1min, `git worktree add ${olderVersion}`, isSmallScreen);
    } else {
    ...
    // Gather each version one at a time so both full node_modules trees never
    // coexist. `npm ls` (inside getDependencies) runs on the FULL tree here;
    // afterwards nothing reads older's node_modules, and only newer's top-level
    // package.json files are read (changelog repo URLs). With minimizeDisk we
    // therefore drop older's tree and prune newer's to just package.json,
    // keeping peak disk to roughly one tree per project.
    console.log('Installing and scanning older version...');
    ...
    const olderDevDeps = ignoreDevDependencies ? await collectAllDevDependencies(olderVersionDir) : [];
    const olderDeps = await getDependencies(olderVersionDir, namespace);
    if (minimizeDisk) {
      await rm(join(olderVersionDir, 'node_modules'), { recursive: true, force: true });
    }

    console.log('Installing and scanning newer version...');
    ...
    const newerDevDeps = ignoreDevDependencies ? await collectAllDevDependencies(newerVersionDir) : [];
    const newerDeps = await getDependencies(newerVersionDir, namespace);
    if (minimizeDisk) {
      await pruneNodeModulesToPackageJson(join(newerVersionDir, 'node_modules'));
    }
    ...
    // Read .dcrignore from the repository root and merge with any extra ignore
    // rules supplied via options (e.g. the .dcr.json `ignore` list).
    console.log('Checking for .dcrignore file...');
    const baseDcrIgnore = await getDcrIgnoreList(repoDir);
    const dcrIgnoreExact = new Set([...baseDcrIgnore.exactMatches, ...extraIgnore.exactMatches]);
    const dcrIgnorePatterns = [...new Set([...baseDcrIgnore.patterns, ...extraIgnore.patterns])];
    ...
    if (ignoreDevDependencies) {
      // Combine dev dependencies (collected during the per-version gather above)
    ...
    const { changelogs, errors, ciStatus } = await getChangelogs(allChangedPackages, newerVersionDir, reposDir, { concurrency: changelogConcurrency, quiet: quietProgress, cloneCache: changelogCloneCache });
    ...
    if (comparison.modified.length > 0) {
      ...
      if (modifiedDepsForChangelog.length > 0) {
        const { changelogs: modifiedChangelogs, errors: modifiedErrors, ciStatus: modifiedCiStatus } = 
          await getChangelogs(modifiedDepsForChangelog, newerVersionDir, reposDir, { concurrency: changelogConcurrency, quiet: quietProgress, cloneCache: changelogCloneCache });
    ...
    if (comparison.nested.upgraded.length > 0) {
      ...
      if (nestedUpgradedDeps.length > 0) {
        const { changelogs: nestedChangelogs, errors: nestedErrors, ciStatus: nestedCiStatus } = 
          await getChangelogs(nestedUpgradedDeps, newerVersionDir, reposDir, { concurrency: changelogConcurrency, quiet: quietProgress, cloneCache: changelogCloneCache });
    ...
    if (generateFullInventory && fullInventory) {
      ...
      if (foundMissedUpgrades.length > 0) {
        ...
        const { changelogs: missedChangelogs, errors: missedErrors, ciStatus: missedCiStatus } = 
          await getChangelogs(missedUpgradesForChangelog, newerVersionDir, reposDir, { concurrency: changelogConcurrency, quiet: quietProgress, cloneCache: changelogCloneCache });
    ...
    try {
      ...
      if (currentBranchName !== olderVersion) {
        try {
          await executeCommand('git', ['worktree', 'remove', olderVersionDir], repoDir, time_1min, 'cleanup older worktree', false);
        } catch (removeError) {
          // If normal remove fails, try with --force
          await executeCommand('git', ['worktree', 'remove', '--force', olderVersionDir], repoDir, time_1min, 'force cleanup older worktree', false);
        }
      }
      if (currentBranchName !== newerVersion) {
        try {
          await executeCommand('git', ['worktree', 'remove', newerVersionDir], repoDir, time_1min, 'cleanup newer worktree', false);
        } catch (removeError) {
          // If normal remove fails, try with --force
          await executeCommand('git', ['worktree', 'remove', '--force', newerVersionDir], repoDir, time_1min, 'force cleanup newer worktree', false);
        }
      }
    } catch (cleanupError) {
