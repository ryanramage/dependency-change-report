 ...
 */
const processSingleDependency = async (dep, newerVersionDir, reposDir, multibar, maxNameLength, useProgressBars = true, quiet = false) => {
 ...
 */
const processInParallel = async (dependencies, newerVersionDir, reposDir, concurrency = 5, quiet = false) => {
  ...
  const promises = dependencies.map(dep => 
    queue.add(() => processSingleDependency(dep, newerVersionDir, reposDir, multibar, maxNameLength, useProgressBars, quiet))
  );
 ...
 */
const getChangelogs = async (upgradedDeps, newerVersionDir, reposDir, { concurrency = 5, quiet = false } = {}) => {
  return await processInParallel(upgradedDeps, newerVersionDir, reposDir, concurrency, quiet);
};
...
export const analyzeDependencyChanges = async (repoUrl, olderVersion, newerVersion, workingDir = process.cwd(), namespace = null, ignoreDevDependencies = false, debugTree = false, cleanupWorktrees = false, generateFullInventory = true, options = {}) => {
  ...
  try {
    ...
    const { changelogs, errors, ciStatus } = await getChangelogs(allChangedPackages, newerVersionDir, reposDir, { concurrency: changelogConcurrency, quiet: quietProgress });
    ...
    if (comparison.modified.length > 0) {
      ...
      if (modifiedDepsForChangelog.length > 0) {
        const { changelogs: modifiedChangelogs, errors: modifiedErrors, ciStatus: modifiedCiStatus } = 
          await getChangelogs(modifiedDepsForChangelog, newerVersionDir, reposDir, { concurrency: changelogConcurrency, quiet: quietProgress });
    ...
    if (comparison.nested.upgraded.length > 0) {
      ...
      if (nestedUpgradedDeps.length > 0) {
        const { changelogs: nestedChangelogs, errors: nestedErrors, ciStatus: nestedCiStatus } = 
          await getChangelogs(nestedUpgradedDeps, newerVersionDir, reposDir, { concurrency: changelogConcurrency, quiet: quietProgress });
    ...
    if (generateFullInventory && fullInventory) {
      ...
      if (foundMissedUpgrades.length > 0) {
        ...
        const { changelogs: missedChangelogs, errors: missedErrors, ciStatus: missedCiStatus } = 
          await getChangelogs(missedUpgradesForChangelog, newerVersionDir, reposDir, { concurrency: changelogConcurrency, quiet: quietProgress });
