...
export const analyzeDependencyChanges = async (repoUrl, olderVersion, newerVersion, workingDir = process.cwd(), namespace = null, ignoreDevDependencies = false) => {
  ...
  try {
    ...
    const comparison = compareDependencies(olderDeps, newerDeps, packageVersions, ignoreDevDependencies);
    ...
    if (ignoreDevDependencies) {
      allChangedPackages = allChangedPackages.filter(dep => !dep.devDep);
    }
        ...
        if (versionInfo.changeType === 'upgraded') {
          try {
            ...
            if (repoUrl) {
              ...
              if (!ignoreDevDependencies || !lockFileDep.devDep) {
    ...
    if (comparison.modified.length > 0) {
      ...
      // Filter out dev dependencies if ignoreDevDependencies is true
      if (ignoreDevDependencies) {
        modifiedDepsForChangelog = modifiedDepsForChangelog.filter(dep => !dep.devDep);
      }
    ...
    if (comparison.nested.upgraded.length > 0) {
      ...
      // Filter out dev dependencies if ignoreDevDependencies is true
      if (ignoreDevDependencies) {
        nestedUpgradedDeps = nestedUpgradedDeps.filter(dep => !dep.devDep);
      }
