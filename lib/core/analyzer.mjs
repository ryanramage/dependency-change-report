...
import { getDprIgnoreList } from '../utils/dprignore-parser.mjs';
 ...
 */
export const analyzeDependencyChanges = async (repoUrl, olderVersion, newerVersion, workingDir = process.cwd(), namespace = null, ignoreDevDependencies = false, debugTree = false, cleanupWorktrees = false) => {
  ...
  try {
    ...
    // Read .dprignore from current directory (repository root)
    console.log('Checking for .dprignore file...');
    const dprIgnoreList = await getDprIgnoreList(process.cwd());
    
    if (dprIgnoreList.size > 0) {
      const dprIgnoreArray = Array.from(dprIgnoreList).sort();
      console.log(`Found .dprignore file with ${dprIgnoreList.size} packages to ignore:`);
      console.log(`  ${dprIgnoreArray.slice(0, 10).join(', ')}${dprIgnoreArray.length > 10 ? ` ... and ${dprIgnoreArray.length - 10} more` : ''}`);
    }
    
    // Collect dev dependencies if ignoring them
    let allDevDeps = new Set();
    let ignoredDevDependencies = [];
    let ignoredFromDprIgnore = [];
    ...
    // Merge .dprignore list with dev dependencies
    if (dprIgnoreList.size > 0) {
      ignoredFromDprIgnore = Array.from(dprIgnoreList).sort();
      
      // Add .dprignore packages to allDevDeps for filtering
      for (const pkg of dprIgnoreList) {
        allDevDeps.add(pkg);
      }
      
      console.log(`Total packages to ignore (dev deps + .dprignore): ${allDevDeps.size}`);
    }
    ...
    if (comparison.modified.length > 0) {
      ...
      // Filter out dev dependencies if ignoreDevDependencies is true
      if (ignoreDevDependencies) {
        modifiedDepsForChangelog = modifiedDepsForChangelog.filter(dep => !allDevDeps.has(dep.newName));
      }
    ...
    if (comparison.nested.upgraded.length > 0) {
      ...
      // Filter out dev dependencies if ignoreDevDependencies is true
      if (ignoreDevDependencies) {
        nestedUpgradedDeps = nestedUpgradedDeps.filter(dep => !allDevDeps.has(dep.name));
      }
    ...
    const report = {
      ...
      ignoredFromDprIgnore: ignoredFromDprIgnore.length > 0 ? ignoredFromDprIgnore : null
    };
