 ...
 */
export const analyzeDependencyChanges = async (repoUrl, olderVersion, newerVersion, workingDir = process.cwd(), namespace = null, ignoreDevDependencies = false, debugTree = false, cleanupWorktrees = false, generateFullInventory = false) => {
  ...
  try {
    ...
    // Generate full inventory if requested
    let fullInventory = null;
    if (generateFullInventory) {
      console.log('\n📦 Generating full dependency inventory...');
      const oldPackages = await scanNodeModulesRecursive(olderVersionDir);
      const newPackages = await scanNodeModulesRecursive(newerVersionDir);
      
      fullInventory = buildInventoryTable(oldPackages, newPackages);
    ...
    if (generateFullInventory && fullInventory) {
      ...
      if (foundMissedUpgrades.length > 0) {
        ...
        const missedUpgradesForChangelog = foundMissedUpgrades.map(dep => {
          // Determine change type using semver if possible
          let changeType = 'unknown';
          try {
            // Handle version conflicts by taking the first version
            const oldVer = dep.oldVersion.split(',')[0].trim();
            const newVer = dep.newVersion.split(',')[0].trim();
          ...
          return {
            name: dep.name,
            oldVersion: dep.oldVersion.split(',')[0].trim(), // Take first version if conflict
            newVersion: dep.newVersion.split(',')[0].trim(), // Take first version if conflict
            changeType: changeType,
            devDep: false, // We know it's not a dev dep since it wasn't filtered
            fromMissedUpgrades: true // Flag to track origin
          };
        });
