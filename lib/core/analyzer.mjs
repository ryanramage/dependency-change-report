...
import { getDcrIgnoreList } from '../utils/dprignore-parser.mjs';
...
export const analyzeDependencyChanges = async (repoUrl, olderVersion, newerVersion, workingDir = process.cwd(), namespace = null, ignoreDevDependencies = false, debugTree = false, cleanupWorktrees = false) => {
  ...
  try {
    ...
    const dcrIgnoreList = await getDcrIgnoreList(process.cwd());
    
    if (dcrIgnoreList.size > 0) {
      const dcrIgnoreArray = Array.from(dcrIgnoreList).sort();
      console.log(`Found .dcrignore file with ${dcrIgnoreList.size} packages to ignore:`);
      console.log(`  ${dcrIgnoreArray.slice(0, 10).join(', ')}${dcrIgnoreArray.length > 10 ? ` ... and ${dcrIgnoreArray.length - 10} more` : ''}`);
    }
    ...
    if (dcrIgnoreList.size > 0) {
      ignoredFromDcrIgnore = Array.from(dcrIgnoreList).sort();
      
      // Add .dcrignore packages to allDevDeps for filtering
      for (const pkg of dcrIgnoreList) {
        allDevDeps.add(pkg);
      }
      
      console.log(`Total packages to ignore (dev deps + .dcrignore): ${allDevDeps.size}`);
    }
