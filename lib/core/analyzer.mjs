...
export const analyzeDependencyChanges = async (repoUrl, olderVersion, newerVersion, workingDir = process.cwd(), namespace = null, ignoreDevDependencies = false, debugTree = false, cleanupWorktrees = false, generateFullInventory = false) => {
  ...
  try {
    ...
    if (generateFullInventory) {
      ...
      // Second pass: Check for missed upgrades
      console.log('\n🔍 Cross-checking inventory against detected changes...');
      const missedUpgrades = [];
      ...
      if (missedUpgrades.length > 0) {
        console.log(`\n⚠️  Found ${missedUpgrades.length} upgraded dependencies that were missed in first pass:`);
        missedUpgrades.forEach(dep => {
          console.log(`\n📦 ${dep.name}`);
        ...
        // Add to report for further analysis
        report.missedUpgrades = missedUpgrades;
      } else {
    ...
    const report = {
      ...
      missedUpgrades: null // Will be populated by full inventory check if any found
    };
