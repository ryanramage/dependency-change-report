...
export const analyzeDependencyChanges = async (repoUrl, olderVersion, newerVersion, workingDir = process.cwd(), namespace = null, ignoreDevDependencies = false, debugTree = false, cleanupWorktrees = false, generateFullInventory = true, options = {}) => {
  ...
  try {
    ...
    if (olderIsCurrentBranch) {
      ...
      await executeCommand('git', ['worktree', 'add', newerVersionDir, newerVersion], repoDir, time_1min, `git worktree add ${newerVersion}`, isSmallScreen);
      ...
      await executeCommand('git', ['worktree', 'add', olderVersionDir, olderVersion], repoDir, time_1min, `git worktree add ${olderVersion}`, isSmallScreen);
    } else {
