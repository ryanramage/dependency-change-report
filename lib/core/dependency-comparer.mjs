...
export const compareDependencies = (oldDeps, newDeps, packageVersions = {}, allDevDeps = new Set()) => {
  ...
  for (const [baseName, data] of potentialNamespaceChanges.entries()) {
    if (data.type === 'added') {
      added.push({
        name: data.newName,
        // Fall back to the lockfile version when the dep tree didn't carry one
        // (npm ls can omit versions for deduped/unmet nodes).
        version: data.newVersion || packageVersions[data.newName]?.newVersion || null,
      ...
      removed.push({
        name: data.oldName,
        version: data.oldVersion || packageVersions[data.oldName]?.oldVersion || null,
